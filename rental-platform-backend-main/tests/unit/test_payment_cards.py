...
def test_delete_payment_card(db: Session):
    ...
    wrong_user_delete = payment_card.delete_by_id_and_user(
        db=db, id=other_card.id, user_id=1
    )
    assert wrong_user_delete is None

    # 验证其他用户的卡未被删除
    other_card_check = payment_card.get_by_id_and_user(
        db=db, id=other_card.id, user_id=2
    )
