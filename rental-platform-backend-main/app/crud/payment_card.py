...
class CRUDPaymentCard(CRUDBase[PaymentCard, PaymentCardCreate, PaymentCardUpdate]):
    def create_with_user(
        self, db: Session, *, obj_in: PaymentCardCreate, user_id: int
    ) -> PaymentCard:
        """
        创建用户的支付卡
        """
    ...
    def get_by_id_and_user(
        ...
        return (
            db.query(PaymentCard)
            .filter(PaymentCard.id == id, PaymentCard.user_id == user_id)
            .first()
        )
    ...
    def get_default_card(self, db: Session, *, user_id: int) -> Optional[PaymentCard]:
        ...
        return (
            db.query(PaymentCard)
            .filter(PaymentCard.user_id == user_id, PaymentCard.is_default is True)
            .first()
        )
    ...
    def update_card(
        ...
        *,
        db_obj: PaymentCard,
        obj_in: Union[PaymentCardUpdate, Dict[str, Any]],
    ) -> PaymentCard:
        """
        更新支付卡信息
        """
    ...
    def set_default_card(self, db: Session, *, card: PaymentCard) -> PaymentCard:
        """
        将指定的支付卡设置为用户的默认卡
        """
        # 1. 将用户的所有其他卡设置为非默认
        db.query(PaymentCard).filter(
            PaymentCard.user_id == card.user_id,
        ).update({PaymentCard.is_default: False})
    ...
    def delete_by_id_and_user(
        ...
        if obj.is_default:
            ...
            other_card = (
                db.query(PaymentCard)
                .filter(PaymentCard.user_id == user_id, PaymentCard.id != id)
                .first()
            )
