...
class CRUDRevenueStats(CRUDBase[RevenueStats, RevenueStatsCreate, dict]):
    ...
    def get_date_range(
        ...
        """获取指定日期范围内的收入统计数据"""
        return (
            db.query(RevenueStats)
            .filter(RevenueStats.date >= start_date, RevenueStats.date <= end_date)
            .order_by(RevenueStats.date)
            .all()
        )

    def create_or_update_daily_stats(
        self, db: Session, *, stats_date: date
    ) -> RevenueStats:
        """创建或更新指定日期的收入统计数据"""
        # 查找是否已存在该日期的统计数据
    ...
    def generate_weekly_stats(
        self, db: Session, *, end_date: date = None
    ) -> RevenueSummary:
        """生成最近一周的收入统计汇总"""
        if not end_date:
            end_date = date.today()
        ...
        total_rentals = 0
        revenue_by_period = {
            RentalPeriod.ONE_HOUR.value: {
                "count": 0,
                "revenue": 0.0,
                "average_daily": 0.0,
            },
            RentalPeriod.FOUR_HOURS.value: {
                "count": 0,
                "revenue": 0.0,
                "average_daily": 0.0,
            },
            RentalPeriod.ONE_DAY.value: {
                "count": 0,
                "revenue": 0.0,
                "average_daily": 0.0,
            },
            RentalPeriod.ONE_WEEK.value: {
                "count": 0,
                "revenue": 0.0,
                "average_daily": 0.0,
            },
        ...
        for period in revenue_by_period:
            revenue_by_period[period]["average_daily"] = (
                revenue_by_period[period]["revenue"] / days_count
                if days_count > 0
                else 0
            )
    ...
    def generate_custom_period_stats(
        self, db: Session, *, start_date: date, end_date: date
    ) -> RevenueSummary:
        """生成自定义日期范围的收入统计汇总"""
        # 确保开始日期不晚于结束日期
