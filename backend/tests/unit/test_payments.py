...
def test_create_payment(db: Session):
    ...
    db_payment = payment.create_with_user_and_rental(
        db=db, obj_in=payment_in, user_id=1
    )
...
def test_get_payment_by_id_and_user(db: Session):
    ...
    created_payment = payment.create_with_user_and_rental(
        db=db, obj_in=payment_in, user_id=1
    )

    # 获取支付记录
    fetched_payment = payment.get_by_id_and_user(
        db=db, id=created_payment.id, user_id=1
    )
    ...
    other_payment = payment.create_with_user_and_rental(
        db=db, obj_in=other_payment_in, user_id=2
    )

    wrong_user_payment = payment.get_by_id_and_user(
        db=db, id=other_payment.id, user_id=1
    )
