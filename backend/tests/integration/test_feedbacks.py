...
def test_rental(client, test_user, test_scooter):
    ...
    response = client.post(
        "/api/v1/rentals/", json=rental_data, headers=test_user["headers"]
    )
    assert response.status_code == status.HTTP_201_CREATED
...
def test_feedback(client, test_user, test_scooter, test_rental):
    ...
    response = client.post(
        "/api/v1/feedbacks/", json=feedback_data, headers=test_user["headers"]
    )
    assert response.status_code == status.HTTP_201_CREATED
...
def test_create_feedback(client, test_user, test_scooter, test_rental):
    ...
    assert (
        data["status"] == FeedbackStatus.IN_PROGRESS.value
    )  # 高优先级自动标记为处理中
    assert "id" in data
...
def test_read_feedbacks(client, test_user, test_feedback):
    """测试获取用户的所有反馈"""
    response = client.get("/api/v1/feedbacks/", headers=test_user["headers"])
...
def test_read_feedback(client, test_user, test_feedback):
    """测试获取单个反馈"""
    response = client.get(
        f"/api/v1/feedbacks/{test_feedback['id']}", headers=test_user["headers"]
    )
...
def test_read_feedback_not_found(client, test_user):
    """测试获取不存在的反馈"""
    response = client.get("/api/v1/feedbacks/9999", headers=test_user["headers"])
...
def test_update_feedback(client, test_user, test_feedback):
    """测试更新反馈"""
    update_data = {"feedback_detail": "更新后的反馈详情"}
...
def test_update_feedback_not_found(client, test_user):
    ...
    response = client.put(
        "/api/v1/feedbacks/9999", json=update_data, headers=test_user["headers"]
    )
...
def test_read_all_feedbacks(client, test_user, test_feedback):
    """测试获取所有反馈（管理员）"""
    # 注意：这里假设测试用户有管理员权限
    response = client.get("/api/v1/feedbacks/admin/all", headers=test_user["headers"])
...
def test_read_high_priority_feedbacks(client, test_user, test_feedback):
    ...
    # 注意：这里假设测试用户有管理员权限
    response = client.get(
        "/api/v1/feedbacks/admin/high-priority", headers=test_user["headers"]
    )
