...
class CRUDPayment(CRUDBase[Payment, PaymentCreate, PaymentUpdate]):
    def create_with_user_and_rental(self, db: Session, *, obj_in: PaymentCreate, user_id: int) -> Payment:
        """
        创建支付记录
        """
        db_obj = Payment(
    ...
    def get_by_id_and_user(self, db: Session, *, id: int, user_id: int) -> Optional[Payment]:
        """
        根据ID和用户ID获取支付记录
        """
        return db.query(Payment).filter(Payment.id == id, Payment.user_id == user_id).first()
    
    def update_payment_status(self, db: Session, *, db_obj: Payment, status: PaymentStatus, transaction_id: Optional[str] = None) -> Payment:
        """
        更新支付状态
        """
